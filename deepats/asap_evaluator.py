...
from deepats.my_kappa_calculator import linear_weighted_kappa as lwk

logger = logging.getLogger(__name__)

class Evaluator():
	
	def __init__(self, dataset, prompt_id, out_dir, dev_x, test_x, dev_y, test_y, dev_y_org, test_y_org, model_id='', N=1, L=0):
		...
		self.best_dev = -1#[-1, -1, -1, -1]
		self.best_test = -1#[-1, -1, -1, -1]
		...
		self.N=N; self.L=L
		if N>1:
			self.dev_y_hot = np.eye(N)[dev_y_org-L]#.astype('float32')
			self.test_y_hot = np.eye(N)[test_y_org-L]#.astype('float32')
			self.dev_y = self.dev_y_hot
			self.test_y = self.test_y_hot
	...
	def evaluate(self, model, epoch, print_info=False):
		...
		#self.dev_metric = self.dev_metric[0]
		#self.test_metric = self.test_metric[0]
		...
		if self.N>1:
			self.dev_pred = np.argmax(self.dev_pred, axis=1) + self.L
			self.test_pred = np.argmax(self.test_pred, axis=1) + self.L
		else:
		...
		'''
		if self.dev_metric > self.best_dev:
			self.best_dev = self.dev_metric
			self.best_test = self.test_metric
			self.best_dev_epoch = epoch
			model.save_weights(self.out_dir + '/best_model_weights.h5', overwrite=True)
			self.flip = True
		if self.test_metric > self.best_test_missed:
			self.best_test_missed = self.test_metric
			self.best_dev_missed = self.dev_metric
			self.best_test_missed_epoch = epoch
			self.flip = True
	...
	def output_info(self):
		print('[CURRENT  %s]	epoch: %i, dev-qwk= %.4f, test-qwk= %.4f' % (self.model_id, self.cur_epoch, self.dev_metric, self.test_metric))
		if self.flip:
			self.flip = False
			print('[BEST DEV %s]	epoch: %i, dev-qwk= %.4f, test-qwk= %.4f' % (self.model_id, self.best_dev_epoch, self.best_dev, self.best_test))
			print('[BEST TEST %s]	epoch: %i, dev-qwk= %.4f, test-qwk= %.4f' % (self.model_id, self.best_test_missed_epoch, self.best_dev_missed, self.best_test_missed))
	
	def print_info(self):
		logger.info('[CURRENT %s]	epoch: %i, dev-qwk= %.4f, test-qwk= %.4f' % (self.model_id, self.cur_epoch, self.dev_metric, self.test_metric))
		logger.info('[BEST DEV %s]	epoch: %i, dev-qwk= \033[92m%.4f\033[0m, test-qwk= %.4f' % (self.model_id, self.best_dev_epoch, self.best_dev, self.best_test))
		logger.info('[BEST TEST %s]	epoch: %i, dev-qwk= %.4f, test-qwk= \033[92m%.4f\033[0m' % (self.model_id, self.best_test_missed_epoch, self.best_dev_missed, self.best_test_missed))
# 		logger.info('[Dev]   loss: %.4f, dmetric: %.4f, qwk: %.4f' % (self.dev_loss, self.dev_metric, self.dev_qwk))
# 		logger.info('[Test]  loss: %.4f, tmetric: %.4f, qwk: %.4f' % (self.test_loss, self.test_metric, self.test_qwk))
# 		logger.info('[BEST DEV] epoch: %i, dev-metric: \033[92m%.4f\033[0m, test-metric: %.4f' % (self.best_dev_epoch, self.best_dev[0], self.best_test[0]))
# 		logger.info('[BEST TEST] epoch: %i, dev-metric: %.4f, test-metric: \033[92m%.4f\033[0m' % (self.best_test_missed_epoch, self.best_dev_missed, self.best_test_missed))
		logger.info('----------------------------------------------------------------')
	
	def print_final_info(self):
		logger.info('----------------------------------------------------------------')
# 		logger.info('[BEST DEV] epoch: %i, dev-metric: %.4f, test-metric: %.4f' % (self.best_dev_epoch, self.best_dev[0], self.best_test[0]))
# 		logger.info('[BEST TEST] epoch: %i, dev-metric: %.4f, test-metric: %.4f' % (self.best_test_missed_epoch, self.best_dev_missed, self.best_test_missed))
		logger.info('[BEST DEV %s]	epoch: %i, dev-qwk= \033[92m%.4f\033[0m, test-qwk= %.4f' % (self.model_id, self.best_dev_epoch, self.best_dev, self.best_test))
		logger.info('[BEST TEST %s]	epoch: %i, dev-qwk= %.4f, test-qwk= \033[92m%.4f\033[0m' % (self.model_id, self.best_test_missed_epoch, self.best_dev_missed, self.best_test_missed))
	
	'''
	def print_info(self):
		logger.info('[Dev]   loss: %.4f, dmetric: %.4f, mean: %.3f (%.3f), stdev: %.3f (%.3f)' % (
			self.dev_loss, self.dev_metric, self.dev_pred.mean(), self.dev_mean, self.dev_pred.std(), self.dev_std))
		logger.info('[Test]  loss: %.4f, tmetric: %.4f, mean: %.3f (%.3f), stdev: %.3f (%.3f)' % (
